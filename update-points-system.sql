-- ============================================
-- UPDATE POINTS SYSTEM - DROP OLD AND CREATE NEW
-- Run this to update the points system with new milestone logic
-- ============================================

-- STEP 1: Drop all old triggers
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_user_created_award_referral_points') THEN
    ...
    RAISE NOTICE '✅ Dropped on_user_created_award_referral_points';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_bet_placed_update_volume') THEN
    ...
    RAISE NOTICE '✅ Dropped on_bet_placed_update_volume';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_bet_placed_check_referral_bonus') THEN
    ...
    RAISE NOTICE '✅ Dropped on_bet_placed_check_referral_bonus';
  END IF;
  
  RAISE NOTICE 'All old triggers dropped';
END $$;

-- STEP 2: Recreate function to award referral points (+10 when someone uses my code)
...
-- STEP 3: Recreate trigger for referral points
CREATE TRIGGER on_user_created_award_referral_points
AFTER INSERT ON users
FOR EACH ROW
EXECUTE FUNCTION award_referral_points();

-- STEP 4: Recreate function to update trading volume and award +1 per $10
...
-- STEP 5: Recreate trigger for trading volume update
...
-- STEP 6: Recreate function to check referral $100 milestones (NEW LOGIC: every $100)
...
-- STEP 7: Recreate trigger to check referral $100 bonus
...
DO $$
BEGIN
  RAISE NOTICE '✅ Points system updated successfully!';
