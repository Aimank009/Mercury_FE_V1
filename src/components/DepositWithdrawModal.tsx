...
export default function DepositWithdrawModal() {
  ...
  useEffect(() => {
    const fetchTokenBalance = async () => {
      ...
      // Don't fetch until wallet client is ready - avoid wasting time on failing RPC calls
      if (!walletClient) {
        console.log('⏳ Waiting for wallet client before fetching balance...');
        setIsFetchingBalance(true);
        return; // Will retry when walletClient becomes available
      }
      ...
      try {
        ...
        if (walletClient && currentChain) {
          try {
            ...
            console.warn('Failed to create provider from wallet client:', err);
          }
        }

        // Fetch balance using wallet provider (or fallback to RPC if provider creation failed)
  ...
  // Note: Retry logic removed - the main useEffect now waits for walletClient before fetching
  // This eliminates unnecessary RPC calls that were failing
